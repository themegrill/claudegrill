#!/usr/bin/env node
/**
 * Set a product up for claudegrill, one idempotent step at a time.
 *
 * This is the deterministic half of `/claudegrill:setup`. Everything here is
...
function writeEnv() {
  ...
  if (!isIgnored(root, rule)) {
    ...
    fs.appendFileSync(gitignore, `${sep}\n# claudegrill — local credentials, never commit\n${rule}\n`);
    addedRule = true;
  }
  if (!isIgnored(root, rule)) {
    return {
  ...
  const lines = existing === "" ? ["# claudegrill — local only. Gitignored, never committed."] : existing.replace(/\n+$/, "").split("\n");
