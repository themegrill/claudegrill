 ...
 * clone, or a CI checkout. `CLAUDEGRILL_HOME` is honoured only as a fallback,
...
export function resolveQaHome(here, up = 1) {
  ...
  // `CLAUDEGRILL_HOME` is the name; `THEMEGRILL_QA_HOME` is what it was called
  // before the rename and is still honoured, because a developer who exported it
  // in a shell profile months ago should not have their setup break silently.
  const env = process.env.CLAUDEGRILL_HOME ?? process.env.THEMEGRILL_QA_HOME;
  if (env) {
    for (const candidate of [
      path.resolve(env),
      path.resolve(env, "plugins", "claudegrill"),
      // The pre-rename layout, for a checkout that has not been pulled yet.
